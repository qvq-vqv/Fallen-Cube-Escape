...
### 📢 [Codex 执行同步] L01-L06 朋友试玩反馈修复边界 (2026-06-29)
* **发信人 (Sender)**: Codex
* **发信时间 (Timestamp)**: 2026-06-29 02:08:29 CST -> 2026-06-29 02:49:12 CST
* **当前状态 (Status)**: `[STATUS: WAITING_FOR_QA]`
* **关联版本 (Git Commit)**: 416415d
* **接棒人 (Next Action)**: QA / Gemini / Antigravity 进行人工试玩与截图反馈；不要改核心代码口径

#### ✅ 用户与 Codex 已确认的本轮硬边界
1. 本轮不是重构前六关，而是修补 L01-L06 的可理解性和硬 Bug；不得删除已有可工作的教学流程。
2. L01 出口展示阶段锁输入约 1 秒，强化出口/传送门高亮；随后进入缩放/视角教学，不允许玩家乱拖导致错过出口。
3. L01 缩放教程只由滚轮/触控板捏合触发；拖拽视角绝不能算缩放。鼓励缩小看全貌，放大不推进教程。
4. L02 中文环境只显示“信任度”，英文环境只显示 `Trust`；不要中英混排。Trust 机制使用现有代码，不重做。
5. Trust 扣分不包含乱点空格/误触；走向明显危险、故意撞怪、死亡等才应影响信任。低信任逆反沿用现有安全邻格逻辑，不许让 Dawn 主动送死。
6. 前期怪物不是 CD 制：第一幕玩家每走一格，敌人走一格。不要把前六关改回实时 CD。
7. Twist 成功一次等价于给敌人一个行动回合；不写 AP/CD/行动点，不显示“消耗 1 移动”。若因此被抓，允许失败。
8. 守钥者机制不得乱改：未拿钥匙主要守钥；Dawn 到钥匙面会被追；钥匙被旋转带走可追钥匙；拿钥匙后按既有狂暴/追人/守门规则处理。
9. L03 首次怪物剧情杀可实装为一次强教学：Dawn 自行靠近怪物，被抓后触发 Glitch 回退；回退后加 Trust，不扣 Trust，并恢复到剧情杀前状态。
10. L04 只改教程目标、高亮和提示，不乱改地图结构；L04 本身无敌人，Twist 算回合不会惩罚新手。
11. L06 当前教程目标导致卡死，必须改为真实相邻格分步引导；可加短提示解释“守钥者被引开”，但只解释既有行为，不改守钥 AI。
12. 全项目扫描并清理玩家可见专业词：禁止 `AP`、`CD`、`Y1`、`CCW`、`CW`、`展开图`、`Net view` 等开发者词出现在玩家可见文本中。剧情语气精修交给后续 Gemini 文案轮，但代码侧先清硬词。

#### 🤝 协同说明
- Gemini/Antigravity 可继续做：原聊天归档、Dawn 文案、人设台词、验收清单、截图反馈整理。
- Gemini/Antigravity 本轮不要再改核心代码规则口径；若后续讨论，请以上 12 条为准，避免把旧 M11 文档中的 CD/AP 词带回前六关。

#### ✅ Codex 物理交付摘要
1. L01 出口展示支持短锁输入；缩放教程改为只认滚轮/触控板向外缩放，拖拽视角不再推进教程。
2. L02 中文统一为“信任度”，英文保留 `Trust`；说明改为“信任低会犹豫/选安全邻格”，不重做现有 Trust 逻辑。
3. L03 前期敌人改为玩家行动后延迟行动；首次追击者接触可触发一次 Glitch 回退，回退不扣信任并加信任。
4. 第一幕保留非 CD 节拍：前 12 关不让敌人按后台实时钟主动行动；玩家移动或成功 Twist 后才触发敌人行动。
5. L06 教程目标改为真实相邻格分步引导，避免“目标不可达”硬卡死。
6. 清理关键玩家可见硬词：`AP`、`CD`、`Y1`、`CCW`、`CW`、`展开图`、`Net view` 等不再出现在主体验文本中；内部变量名未重命名，避免引入无意义风险。

#### 🧪 验证结果
- `npm run check`: PASS
- `npm run audit:levels -- "L0[1-6]"`: PASS
- `npm run audit:quality`: PASS，0 issue / 0 warning，仅 L32/L33 两条 info，与本轮无关。
- `npm run audit:design`: PASS
- `npm run smoke:browser`: SKIPPED，原因是工作区未安装 Playwright。需要 QA 人工在浏览器试玩 L01-L06。

#### 👀 QA 建议
1. L01：拖拽视角不应推进缩放教程；向外滚动/触控板缩小才推进。
2. L03：玩家走一步后，追击者应有短延迟再走一步；首次被追击者碰到应触发回退而不是直接结算。
3. L04/L06：Twist 后若有敌人，应给敌人一个行动节拍；L06 教程目标必须每一步都能点到相邻格。
